...
router.post('/new', passport.authenticate('jwt', { session: false }), async (req, res) => {
	...
	if (!currentPrice) {
		...
		validationErrors.alertType = `${alert.alertType} alert does not make sense for current price ${currentPrice}`;
	}
...
// @access  Private
router.delete('/:alertId', passport.authenticate('jwt', { session: false }), async (req, res) => {
	try {
		const alert = await Alert.findOneAndRemove({ _id: req.params.alertId, user: req.user.id });
		if (!alert) {
			res
				.status(400)
				.json({ error: `no alert with id: ${req.params.alertId}, user: ${req.user.id} found to delete` });
			return;
		}
